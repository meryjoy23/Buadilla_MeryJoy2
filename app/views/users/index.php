  ...
  <nav class="bg-gradient-to-r from-[#4e342e] via-[#6d4c41] to-[#3e2723] shadow-lg border-b-4 border-[#a1887f]">
    <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
      <h1 class="text-[#f5e6ca] font-title text-2xl flex items-center gap-2">
        <i class="fa-solid fa-mug-saucer"></i> BrewEase Student Directory
      </h1>
    </div>
  </nav>
    ...
    <button type="submit" class="bg-[#6d4c41] hover:bg-[#4e342e] text-[#f5e6ca] px-4 py-2 rounded-r-lg shadow transition-all duration-300">
      <i class="fa fa-search"></i>
        ...
        <a href="<?=site_url('users/create')?>"
           class="btn-hover inline-flex items-center gap-2 bg-gradient-to-r from-[#4e342e] to-[#8d6e63] text-[#f5e6ca] font-bold px-5 py-2 rounded-lg shadow-md transition-all duration-300">
        ...
        <table class="w-full text-center border-collapse">
          ...
          <tbody class="text-[#3e2723] text-sm" style="font-family:'Quicksand', sans-serif;">
            <?php if(!empty($users)): ?>
                ...
                <tr class="hover:bg-[#d7ccc8] transition duration-200">
                  ...
                  <td class="py-3 px-4 flex justify-center gap-3">
                    <a href="<?=site_url('users/update/'.$user['id']);?>"
                       class="btn-hover bg-[#6d4c41] hover:bg-[#4e342e] text-[#f5e6ca] px-3 py-1 rounded-lg shadow flex items-center gap-1">
                      <i class="fa-solid fa-pen-to-square"></i> Update
                    </a>
                    <a href="<?=site_url('users/delete/'.$user['id']);?>"
                       class="btn-hover bg-[#3e2723] hover:bg-[#4e342e] text-[#f5e6ca] px-3 py-1 rounded-lg shadow flex items-center gap-1">
              ...
              <?php endforeach; ?>
            <?php else: ?>
              <tr><td colspan="5" class="py-4 text-[#6d4c41]">No students found.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
...
<div class="mt-4 flex justify-center">
  <div class="pagination flex space-x-2">
    ...
    <a href="<?=site_url('auth/logout');?>"
       class="logout-btn ml-4 bg-[#6d4c41] hover:bg-[#4e342e] text-[#f5e6ca] px-4 py-2 rounded-lg font-semibold flex items-center gap-2 transition">
